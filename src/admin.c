/*
 *
 * (c) 2004-2014 Denora Team
 * Contact us at info@denorastats.org
 *
 * Please read COPYING and README for furhter details.
 *
 * Based on the original code of Anope by Anope Team.
 * Based on the original code of Thales by Lucas.
 *
 *
 *
 */

#include "denora.h"

/*************************************************************************/

Dadmin *find_admin_byname(char *mask)
{
	Dadmin *a;
	sqlite3_stmt *stmt;
	sqlite3 *db;
	char **sdata;

	if (!mask)
	{
		return NULL;
	}

	db = DenoraOpenSQL(DenoraDB);
	stmt = DenoraPrepareQuery(db, "SELECT * FROM %s WHERE uname='%q'", AdminTable, mask);
	sdata = DenoraSQLFetchRow(stmt, FETCH_ARRAY_NUM);
	if (sdata) {
		a = make_admin(sdata);
	}
	sqlite3_finalize(stmt);
	DenoraCloseSQl(db);

	if (stricmp(a->name, mask) == 0)
	{
		return a;
	}
	return NULL;
}

/*************************************************************************/

Dadmin *find_admin(char *name, User * u)
{
	Dadmin *a;

	a = find_admin_byname(name);
	
	if (a)
	{
		if (match_usermask(a->hosts, u))
		{
			return a;
		}
	}
	return NULL;
}


/*************************************************************************/

Dadmin *make_admin(char **data)
{
	Dadmin *a;

	a = calloc(sizeof(Dadmin), 1);

	a->name = sstrdup(data[0]);
	a->passwd = sstrdup(data[1]);	
	a->hosts = sstrdup(data[3]);
	a->language = atoi(data[4]);
	a->level = atoi(data[2]);
	a->configfile = atoi(data[5]);

	return a;
}

/*************************************************************************/

int free_admin(Dadmin * a)
{
	int i = 0;

	if (a)
	{
		if (a->hosts)
		{
			free(a->hosts);
		}
		if (a->passwd)
		{
			free(a->passwd);
		}
		if (a->name)
		{
			free(a->name);
		}
		free(a);
		return 1;
	}
	return 0;
}

/*************************************************************************/

int add_sqladmin(char *name, char *passwd, int level, char *host, int lang, int configadmin, int encrypt)
{
	int res;

	res = DenoraSQLQuery(DenoraDB, "INSERT INTO %s (uname, passwd, level, host, lang, config) VALUES ('%q', '%q', %d, %s, %d, %d)", 
		  AdminTable, name, passwd, level, host, lang, configadmin); 

	return res;
}

/*************************************************************************/

int del_sqladmin(char *name)
{
	int res;

	res = 
	
	return res;
}

/*************************************************************************/

int AdminSetPassword(Dadmin * a, char *newpass)
{
	int crypted;

	free(a->passwd);
	a->passwd = sstrdup(MakePassword(newpass));
	crypted = is_crypted(a->passwd);
	DenoraSQLQuery(AdminDB, "UPDATE %s SET passwd=%s%q%s WHERE uname = '%q'", AdminTable, crypted ? "'" : "MD5('", a->passwd, crypted ? "'" : "')", a->name);

	return 1;
}